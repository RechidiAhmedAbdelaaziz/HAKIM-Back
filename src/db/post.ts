...
export const createPost = async (info: CreatePostDio): Promise<any> => {
	...
	if (post) await User.findByIdAndUpdate(post.poster, { $pull: { posts: post._id } });
	return post;
};
...
export const updatePost = async (info: UpdatePostDio): Promise<PostType> => {
	const { id, post, posterId, pullComment, pullLike, pushComment, pushLike } = info;
...
export const deletePost = async (id: string, poster: any): Promise<PostType> => {
